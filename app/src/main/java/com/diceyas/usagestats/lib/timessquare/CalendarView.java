...
public class CalendarView extends LinearLayout {
  ...
  final Map<String, List<Calendar>> markCalsMap = new HashMap<String, List<Calendar>>();
	final Map<String, List<Calendar>> markCalsMap2 = new HashMap<String, List<Calendar>>();
  ...
  public void markDatesOfMonth(int year, int month, 
		  int markIcon, boolean markText, List<Calendar> calendars) {
	  Calendar cal = Calendar.getInstance(locale);
	  cal.set(year, month, 1);
	  
	  String monthLabel = monthNameFormat.format(cal.getTime());
	  if (markIcon == 2) {
		  markCalsMap2.put(monthLabel, calendars);
	  }
  ...
  public List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
	    ...
	    while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
	      ...
	      for (int c = 0; c < 7; c++) {
	        ...
	        int isMark = markCalsContainsDate(cal);
	...
	private int markCalsContainsDate(Calendar cal) {
		List<Calendar> selectedCals = markCalsMap.get(monthNameFormat.format(cal.getTime()));
		List<Calendar> selectedCals2 = markCalsMap2.get(monthNameFormat.format(cal.getTime()));
    	if(selectedCals!= null)if(containsDate(selectedCals, cal))return 1;
		if(selectedCals2!=null)if(containsDate(selectedCals2, cal))return 2;
		return 0;
	}
