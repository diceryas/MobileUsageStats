...
public class LocalDataBase {
    ...
    public static boolean exist(SQLiteDatabase db, String packageName)
    {
        ...
        String command = "select count(*) as cnt " + "from useTime "
    ...
    public static double[] getUsageRatio(SQLiteDatabase db)
    {
        ...
        double percentage[] = new double[]{0.2, 0.1, 0.5, 1, 1, 1, 1, 0.8};
        Random r1 = new Random(System.currentTimeMillis());
        double sum = 0;
        for (int i = 0; i < 8; i++) {
            ratio[i] = r1.nextDouble() * percentage[i];
            sum += ratio[i];
        }
        int useTime[] = new int[8];
        boolean succ = false;
        int tmpTime = totalTime;
        while (!succ) {
            succ = true;
            tmpTime = 0;
            for (int i = 0; i < 8; i++) {
            ...
            for (int i = 0; i < 8; i++) ratio[i] = (double)useTime[i] / tmpTime;
        }
        return ratio;
    }
